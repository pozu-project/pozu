/**
 * Boot script for the box-selection page.
 *
 * Without `?dev-mode` in the URL the page shows a placeholder message.
 * With `?dev-mode` the full box-labeling widget is shown: users can draw
 * rectangles and use all zoom/pan tools, but the "No Subject Present" and
 * "Submit" buttons are permanently disabled so no data is submitted and no
 * frames are skipped.
 */
import "./styles.css";
...
if (!DEV_MODE) {
    ...
    const placeholder = document.getElementById("boxPlaceholder") as HTMLElement;
    const boxView = document.getElementById("boxView") as HTMLElement;
    ...
    function renderBoxOverlay() {
        if (!box) { boxOverlay.style.display = "none"; return; }
        const meta = getVideoMeta();
        if (!meta) { boxOverlay.style.display = "none"; return; }
    ...
    async function loadRandomFrame() {
        ...
        if (total < 2) { await showFrame(0); return; }
